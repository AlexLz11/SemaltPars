...
def get_main_links(url, scheme, session=None):
    soup = get_soup(url, session)
    url_tags = soup.select_one('ul.catalog-section-list-tile-list').select('a.catalog-section-list-item-link')
    main_links = [scheme + a['href'] for a in url_tags]
    return main_links
...
def get_data(url, scheme, session=None):
    ...
    category_name = soup.select_one('h1.bx_catalog_tile_category_title').text
    items_cont = soup.select_one('div.wrapper_product_items')
    if not items_cont:
        cat_links = [scheme + tag.find('a')['href'] for tag in category_cont.select('h2.bx_catalog_tile_title')]
...
if __name__ == '__main__':
    ...
    with requests.Session() as rs:
        main_links = get_main_links(start_url, scheme, rs)    
        products = []
        for link in main_links:
            products = products + get_data(link, scheme, rs)
